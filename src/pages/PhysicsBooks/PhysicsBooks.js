...
import './PhysicsBooks.scss'; // Ensure this file exists
import Footer from "../../components/Footer/Footer";

function PhysicsBooks() {
    ...
    const fetchPlaylists = async () => {
      try {
        const response = await fetch(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&type=playlist&q=physics&channelId=${channelId}&maxResults=50&key=${apiKey}`
        );
  ...
  return (
    <>
      ...
      <div className="hero-playlist">
        <h1 className="title">Physics Playlists</h1>
        {isLoading ? (
